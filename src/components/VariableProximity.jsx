    ...
    letterRefs.current.forEach((letterRef, index) => {
      if (letterRef) {
        try {
          ...
          interpolatedSettingsRef.current[index] = 'wght 400';
        }
      }
    });
  }, [fromFontVariationSettings]);
  ...
  useAnimationFrame(() => {
    ...
    letterRefs.current.forEach((letterRef, index) => {
      ...
      try {
        ...
        // Fallback for browsers that don't support font-variation-settings
        const weight = parsedSettings.find(s => s.axis === 'wght');
        if (weight) {
          const interpolatedValue = weight.fromValue + (weight.toValue - weight.fromValue) * falloffValue;
          letterRef.style.fontWeight = interpolatedValue;
        }
      }
    });
  });
